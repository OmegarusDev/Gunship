...
for (const scenarioId of Object.keys(SCENARIOS)) {
  ...
  for (const seed of [1000, 2000, 3000]) {
    ...
    ok(world.extraction != null, `[${scenarioId}/${seed}] extraction flag present`);
    ok((world.radarSites || []).length >= 1, `[${scenarioId}/${seed}] radar installation present`);

    if (scenarioId === 'strike' || scenarioId === 'sabotage') {
      const t = world.objective.target;
      const inWorld = world.buildings.some((b) => b.id === t.id);
      ok(
        t && t.destructible && t.hp > 0 && inWorld,
        `[${scenarioId}/${seed}] target building exists & destructible`
      );
    } else if (scenarioId === 'intercept') {
      const t = world.objective.target;
      ok(
        t && t.objectiveTarget === true && Array.isArray(t.route) && t.route.length,
      ...
      ok(
        world.supplyCrates.some((c) => c.objective === true),
    ...
    ok(objectiveComplete(world), `[${scenarioId}/${seed}] objective reaches completion state`);
...
ok(
  lv > 0 && cLvl.pilot.level > before && cLvl.pilot.skillPoints >= lv,
...
ok(
  capped.pilot.level === 10 && capped.pilot.xp === xpCap,
...
ok(
  res2.died && career.pilot !== oldPilot && career.dollars === d1 + 300,
...
ok(
  heliPlain.bulletDamage > 0 && heliPlain.maxHp > 0 && typeof heliPlain.critChance === 'number',
