...
function sampleTerrain(wx, wy) {
  ...
  if (_sharedTerrain) {
    ...
    switch (ty) {
      ...
      case 'oasis': { r = BIOME.oasisGreen.r; g = BIOME.oasisGreen.g; b = BIOME.oasisGreen.b; break; }
      case 'hardpack': { r = BIOME.hardpack.r + shade * 0.5; g = BIOME.hardpack.g + shade * 0.5; b = BIOME.hardpack.b + shade * 0.5; break; }
      case 'gravel': { r = BIOME.gravel.r + shade * 0.6; g = BIOME.gravel.g + shade * 0.6; b = BIOME.gravel.b + shade * 0.6; break; }
      case 'dunes': {
        ...
        if (dn < -0.15) { const sh = -dn * 40; r -= sh; g -= sh; b -= sh; }
        break;
      }
      case 'rock': {
...
function updateTerrainGrid(cam) {
  ...
  if (!tgCanvas || cols !== tgCols || rows !== tgRows || x0 !== tgX0 || y0 !== tgY0) {
    ...
    if (!tgCanvas) { tgCanvas = document.createElement('canvas'); tgCtx = tgCanvas.getContext('2d'); }
    if (tgCanvas.width !== cols || tgCanvas.height !== rows) { tgCanvas.width = cols; tgCanvas.height = rows; }
...
function getGrainPattern(ctx) {
  ...
  for (let i = 0; i < 1400; i++) {
    ...
    const light = rng() > 0.5;
    const style = light ? `rgba(255,244,214,${(0.04 + rng() * 0.06).toFixed(3)})` : `rgba(30,22,10,${(0.05 + rng() * 0.07).toFixed(3)})`;
