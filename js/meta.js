...
const NAME_FIRST = ['Tariq','Basim','Daud','Faisal','Hakim','Jalal','Karim','Nasir','Omar','Qasim','Rafi','Sami','Tariq','Zafir','Idris','Majed'];
const NAME_CALL  = ['VIPER','GHOST','SABRE','NOMAD','RAVEN','DAGGER','HAWK','JACKAL','SCORPION','FALCON','MANTIS','DUNE'];
const NAME_LAST  = ['al-Asad','al-Din','Haddad','Karam','Mahmoud','Nazari','Qadir','Sahim','Toma','Zahran','Farouk','Hakimi'];
...
export const SKILL_GRID = [
  ...
  { id: 'doubletap',     branch: 0, tier: 2, name: 'Double Tap',       desc: '15% chance: second projectile' },
  // ── Pilot (control) ──
  ...
  { id: 'interceptor',   branch: 1, tier: 2, name: 'Interceptor',      desc: '+20% tracking on fast targets' },
  ...
  { id: 'targetcomp',    branch: 1, tier: 2, name: 'Targeting Computer', desc: 'Auto-lead moving targets' },
  // ── Recon (awareness) ──
  ...
  { id: 'fullspectrum',  branch: 2, tier: 2, name: 'Full Spectrum',    desc: 'All hostiles visible on radar' },
  { id: 'threatdet',     branch: 2, tier: 0, name: 'Threat Detector',  desc: 'Incoming fire warning flash' },
  { id: 'markedtarget',  branch: 2, tier: 1, name: 'Marked Target',    desc: '+10% damage to locked target' },
  { id: 'reconflyover',  branch: 2, tier: 2, name: 'Recon Flyover',    desc: 'Full radar reveal every 30s (10s)' },
  // ── Thrust (speed) ──
  ...
  { id: 'laststand',     branch: 4, tier: 2, name: 'Last Stand',       desc: 'Survive lethal hit once per sortie' },
];
...
export function gridNeighbors(id) {
  ...
  if (idx < 0) return [];
  const b = Math.floor(idx / 6), t = idx % 6;
  const row = t < 3 ? 0 : 1, col = t % 3;
  const at = (r, c) => {
...
export function canAllocate(allocated, id) {
  ...
  const idx = SKILL_GRID.findIndex(n => n.id === id);
  const t = idx % 6, row = t < 3 ? 0 : 1, col = t % 3;
...
export function aggregateModifiers(pilot, hangar, gunshipId = 'cobra') {
  const m = {
    dmgMult: 1, spreadMult: 1, fireRateMult: 1,
    turnMult: 1, lockMult: 1, autoLead: false,
    accelMult: 1, maxSpeedMult: 1,
    radarRange: 1, detectionMult: 1, fullSpectrum: false, reconPulse: false, markedDmg: 0,
    dmgResist: 0, redScreenRed: 0, bonusHp: 0,
    boostDurMult: 1, boostPotency: 1,
    doubleTap: 0, critChance: 0, sniperBonus: 0,
    lastStand: false, missileWarning: false,
  };

  // Pilot base stats
  const s = pilot.stats;
  m.dmgMult *= 1 + (s.accuracy - 1) * 0.10;
  ...
  m.redScreenRed += (s.grit - 1) * 0.10;

  // Skill nodes
  const has = id => pilot.allocated.includes(id);
  if (has('stabilizer'))   m.spreadMult *= 0.95;
  if (has('marksman'))     m.dmgMult *= 1.10;
  if (has('deadeye'))      m.critChance += 0.15;
  if (has('rapidfire'))    m.fireRateMult *= 0.90;
  if (has('sniper'))       m.sniperBonus += 0.20;
  if (has('doubletap'))    m.doubleTap += 0.15;
  if (has('steadyhands'))  m.turnMult *= 1.10;
  if (has('lockedon'))     m.lockMult *= 1.15;
  if (has('interceptor'))  m.turnMult *= 1.10; // tracking folds into turn
  if (has('recoilcontrol'))m.spreadMult *= 0.85;
  if (has('smoothop'))     m.turnMult *= 1.10;
  if (has('targetcomp'))   m.autoLead = true;
  if (has('sharpeyes'))    m.radarRange *= 1.20;
  if (has('intelnet'))     m.detectionMult *= 1.15;
  if (has('fullspectrum')) m.fullSpectrum = true;
  if (has('threatdet'))    m.missileWarning = true;
  if (has('markedtarget')) m.markedDmg += 0.10;
  if (has('reconflyover')) m.reconPulse = true;
  if (has('lightframe'))   m.accelMult *= 1.10;
  if (has('turbo'))        m.maxSpeedMult *= 1.15;
  if (has('afterburner'))  m.boostDurMult *= 1.20;
  if (has('quickstart'))   m.boostPotency *= 1.25;
  if (has('zoom'))         m.accelMult *= 1.10;
  if (has('overdrive'))    m.maxSpeedMult *= 1.10;
  if (has('hardened'))     m.dmgResist += 0.10;
  if (has('ironskin'))     m.dmgResist += 0.15;
  if (has('bulletproof'))  m.dmgResist += 0.20;
  if (has('steady'))       m.redScreenRed += 0.20;
  ...
  if (lvl('engine') >= 1) { m.maxSpeedMult *= 1.06; m.accelMult *= 1.08; }
  if (lvl('engine') >= 2) { m.maxSpeedMult *= 1.06; m.accelMult *= 1.08; }
  if (lvl('armor') >= 1)  { m.bonusHp += 20; m.dmgResist += 0.05; }
  if (lvl('armor') >= 2)  { m.bonusHp += 20; m.dmgResist += 0.05; }
  ...
  if (lvl('avionics') >= 1) { m.lockMult *= 1.15; m.radarRange *= 1.10; }
  if (lvl('avionics') >= 2) { m.lockMult *= 1.15; m.radarRange *= 1.10; }
...
export function allocateSkill(career, nodeId) {
  ...
  if (!canAllocate(career.pilot.allocated, nodeId)) return { ok: false, reason: 'LOCKED — CONNECT FROM OWNED NODE' };
